...
const getListTicket = async (page, limit, queryObj = {}) => {
  ...
  // Tính số lượng vé sau khi lọc
  const count = await Tickets.countDocuments(queryObj);
...
const TicketController = {
  ...
  getTopUsers: async (req, res) => {
    try {
      // Truy vấn 1: Tính số lượng vé đã đặt với trạng thái "PAID" cho từng người dùng
      const topUsersStats = await Tickets.aggregate([
        {
          $match: { status: "PAID" }, // Lọc chỉ lấy vé có trạng thái "PAID"
        },
        {
          $group: {
