...
const PaymentComponent: React.FC<PaymentComponentProps> = ({ dataTickes }) => {
  const [value, setValue] = useState<string>(""); // Đảm bảo giá trị mặc định không phải là null
  const [timeLeft, setTimeLeft] = useState<number>(600); // Đếm ngược từ 600 giây (10 phút) 10 phút = 600
  const [showDialog, setShowDialog] = useState(false); // Trạng thái hiển thị dialog
  const [paymentStatus, setPaymentStatus] = useState<string>(""); // Trạng thái thanh toán
  const [appTransId, setAppTransId] = useState<string>(""); // Trạng thái thanh toán
  const [paymentMethod, setpaymentMethod] = useState<string>(""); // Trạng thái thanh toán
  ...
  // Thời gian đếm ngược
  const [endTime, setEndTime] = useState<Date | null>(null); // Thời gian kết thúc sau 10 phút

  useEffect(() => {
    if (dataTickes?.createdAt) {
      const createdAtTime = new Date(dataTickes.createdAt); // Lấy thời gian createdAt
      createdAtTime.setMinutes(createdAtTime.getMinutes() + 10); // Thêm 10 phút vào createdAt

      setEndTime(createdAtTime); // Cập nhật thời gian kết thúc
    }
  }, [dataTickes]);
  ...
  useEffect(() => {
    ...
    const timerId = setInterval(() => {
      ...
      if (timeDiff <= 0) {
        setTimeLeft(0); // Dừng đếm ngược khi hết thời gian
        clearInterval(timerId);
      } else {
        setTimeLeft(Math.floor(timeDiff / 1000)); // Cập nhật thời gian còn lại mỗi giây
      }
    }, 1000);

    // Dọn dẹp timer khi component unmount hoặc khi thời gian kết thúc
    return () => clearInterval(timerId);
  }, [endTime]);
  ...
  // Tạo hiệu ứng khi người dùng chọn phương thức thanh toán
  const renderItem = (item: { label: string; value: string; icon: any }) => {
    return (
      <View style={styles.item}>
        <Image source={item.icon} style={styles.iconImage} />
  ...
  const handlePaymentUpdate = async () => {
    try {
      ...
      console.log(value); // Phương thức thanh toán đã chọn
      console.log(id); // ID của vé
      console.log("Token: your-auth-token"); // Token xác thực

      // Gửi phương thức thanh toán lên API và lưu kết quả vào 'result'
      const result = await updatePaymentMethod(id, value, "your-auth-token");
      console.log("Payment method updated successfully:", result);
