...
export const orderApi = createApi({
  ...
  endpoints: (builder) => ({
    ...
    //   providesTags: ['Orders']
    // }),
    getAllOrderPending: builder.query<any, { limit: number; page: number; status?: string }>({
      query: (options) => {
        const { limit, page, status } = options
        let query = `/tickets?limit=${limit}&page=${page}`

        // Nếu có status, thêm nó vào query string
        if (status) {
          query += `&status=${status}`
        }
