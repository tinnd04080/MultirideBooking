...
const TopUsers: React.FC = () => {
  ...
  const fetchTopUsers = async () => {
    try {
      ...
      setData(result.data)
      console.log(`Dữ liệu get được ${data}`)
    } catch (error) {
  ...
  const columns = [
    {
      ...
      render: (_: any, __: any, index: number) => (
        <span>
          {index === 0 ? <CrownTwoTone twoToneColor='#FFD700' style={{ fontSize: '16px' }} /> : null} {index + 1}
        </span>
      )
    },
    {
      ...
      dataIndex: 'name',
      key: 'name',
      render: (text: string, record: UserStats) => (record.name ? text : `User ID: ${record._id}`)
    },
    {
      title: 'Email',
      dataIndex: 'email',
